...
{
    public class AssessmentManager : MonoBehaviour, IHubModeRunner
    {
        [Header("References")]
        public ThesisSessionLogger logger;

        [Header("Config")]
        [Tooltip("If true, loads tasks from Resources/Configs/assessment_tasks_v1.json")]
        ...
        // ===== IHubModeRunner =====
        public string ModeId => "Assessment";
        ...
        private void Awake()
        {
            ...
            {
                Debug.LogWarning(
                    "[Assessment] ThesisSessionLogger not found. " +
                    "Please start Play from ThesisHub and enter Assessment via the Hub button (logger is DontDestroyOnLoad)."
                );
            }
            ...
            if (useConfig)
            {
                _cfg = PracticeConfigLoader.LoadFromResources("Configs/assessment_tasks_v1");
                if (_cfg == null)
                {
                    Debug.LogWarning("[Assessment] Config load failed. Will use default stub task parameters.");
                }
            }
        ...
        public void HubStart() => StartAssessmentSession();
        ...
        // ===== UI callbacks (stable entry points for buttons in the Assessment scene) =====
        public void UIStart() => StartAssessmentSession();
        ...
        public void StartAssessmentSession()
        {
            if (logger == null)
            {
                Debug.LogError("[Assessment] Cannot start session: logger is null.");
                return;
            }

            if (logger.HasActiveSession)
            {
                Debug.LogWarning("[Assessment] Session already active. Use End first.");
                return;
            }

            logger.mode = "Assessment";
        ...
        public void AddFakeTask()
        {
            if (logger == null)
            {
                Debug.LogError("[Assessment] Cannot add task: logger is null.");
                return;
            }

            if (!logger.HasActiveSession)
            {
                Debug.LogWarning("[Assessment] No active session. Press Start first.");
                return;
            }

            // Assessment mode also uses a randomised default success rate.
            bool success = Random.value > 0.35f;
            AddOneTask(success);
        }
        ...
        public void EndAndExport()
        {
            if (logger == null)
            {
                Debug.LogError("[Assessment] Cannot end/export: logger is null.");
                return;
            }
        ...
        public void EndSessionOnly()
        {
            if (logger == null)
            {
                Debug.LogError("[Assessment] logger is null.");
                return;
            }

            if (!logger.HasActiveSession)
            {
                Debug.LogWarning("[Assessment] No active session.");
                return;
            }
        ...
        public void ExportOnly()
        {
            ...
            if (logger.HasActiveSession)
            {
                Debug.LogWarning("[Assessment] End session before export.");
                return;
            }
        ...
        private void AddOneTask(bool success)
        {
            ...
            // 3) Calculate the score and error type with QualityScorerV1.
