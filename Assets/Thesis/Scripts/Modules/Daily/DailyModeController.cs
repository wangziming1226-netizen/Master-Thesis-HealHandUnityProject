...
{
    public enum DailyFeedbackState
    {
        Idle,
        TrackingLost,
        Ready,
    ...
    public class DailyModeController : MonoBehaviour
    {
        [Header("Input")]
        ...
        [Header("Target")]
        public DailyGestureType targetGestureType = DailyGestureType.OpenHand;
        ...
        public string sessionId = "daily_smoke_001";
        public string logFileName = "daily_log.csv";
        ...
        public DailyFeedbackState feedbackState = DailyFeedbackState.Idle;
        public DailyErrorType errorType = DailyErrorType.None;
        ...
        void Awake()
        {
            ...
            if (debugLogs)
            {
                Debug.Log($"[Daily] Awake on {name} (scene={gameObject.scene.name})");
                Debug.Log(gestureInput != null
                    ? $"[Daily] gestureInput bound = {gestureInput.name}"
                    : "[Daily] gestureInput is NULL");
                Debug.Log(gestureRecognizer != null
                    ? $"[Daily] gestureRecognizer bound = {gestureRecognizer.name}"
                    : "[Daily] gestureRecognizer is NULL");
            }
        }
        ...
        void Update()
        {
            ...
            if (state == RunState.Running)
            {
                ...
                if (elapsedTime >= maxTaskSeconds && state == RunState.Running)
                {
                    ...
                    if (!tracking)
                        errorType = DailyErrorType.TrackingLost;
                    else if (holdAccum > 0f || assistAccum > 0f)
                        errorType = DailyErrorType.UnstableHold;
                    else
                        errorType = DailyErrorType.GestureMismatch;
                }
            }
            ...
            if (debugLogs && Time.time >= debugNextTime)
            {
                ...
                Debug.Log(
                    $"[Daily] target={TargetGestureKey}, state={state}, tracking={tracking}, gesture={gesture}, " +
                    $"targetConf={targetConfidence:0.00}, secondConf={secondBestConfidence:0.00}, " +
                    $"strong={isStrongMatch}, weak={isWeakMatch}, hold={holdAccum:0.00}/{targetHoldSeconds:0.00}, " +
                    $"assist={assistAccum:0.00}/{targetHoldSeconds:0.00}, elapsed={elapsedTime:0.00}/{maxTaskSeconds:0.00}, " +
                    $"score={resultScore:0.0}, adaptive={adaptiveEnabled}, level={currentDifficultyLevel}"
                );
            }
        }
        ...
        private void UpdateConfidenceCache(string currentGesture)
        {
            ...
            if (relaxOKThresholds && targetGestureType == DailyGestureType.OK)
            {
        ...
        private void InitTexts()
        {
            if (txtTaskNameValue != null)
                txtTaskNameValue.text = "Daily Task";
        ...
        private void RefreshUnifiedOutput(bool tracking, string gesture)
        {
            ...
            {
                case RunState.Idle:
                    feedbackState = DailyFeedbackState.Idle;
                    errorType = DailyErrorType.None;
                ...
                case RunState.Running:
                    if (!tracking)
                    {
                        feedbackState = DailyFeedbackState.TrackingLost;
                        primaryMessage = "Hand not detected";
                        secondaryMessage = "Please move your hand into view";
                    }
                    else if (isStrongMatch)
                    {
                        feedbackState = DailyFeedbackState.Holding;
                    ...
                    {
                        feedbackState = DailyFeedbackState.Ready;
        ...
        private float GetPenalty(DailyErrorType type)
        {
            switch (type)
            {
                case DailyErrorType.TrackingLost:
        ...
        private void ExportDailyLog()
        {
            try
            {
                string dir = Path.Combine(Application.persistentDataPath, "DailyLogs");
                ...
                using (var sw = new StreamWriter(filePath, true))
                {
                    ...
                    {
                        sw.WriteLine(
                            "session_id,timestamp_utc,module,difficulty_mode,difficulty_level,scanned_card_id,target_gesture,success,score,error_type,feedback_state," +
                ...
                if (debugLogs)
                    Debug.Log($"[Daily] Log exported -> {filePath}");
            }
            catch (Exception ex)
            {
                Debug.LogError($"[Daily] ExportDailyLog failed: {ex.Message}");
            }
        }
        ...
        private void ApplyDifficultyLevel(int level)
        {
            ...
            if (weakMatchThreshold > strongMatchThreshold - 0.10f)
            {
                weakMatchThreshold = Mathf.Clamp(strongMatchThreshold - 0.10f, minWeakThreshold, maxWeakThreshold);
            }
        }
        ...
        private string GetGestureKey(DailyGestureType type)
        {
            switch (type)
            {
                case DailyGestureType.Fist:
        ...
        private string GetGestureDisplayName(DailyGestureType type)
        {
            switch (type)
            {
                case DailyGestureType.Fist:
